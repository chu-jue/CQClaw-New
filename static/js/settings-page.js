(async function(){
  ...
  const settingKeys = ['adbPath','quickOutputDir','localTempDir','agentApkPath','enterpriseSourcePath'];
  function status(msg){ $('settingsStatus').textContent = msg; }
  async function load(){
    const data = await api('/api/settings');
    state.settings = {adbPath:'adb', quickOutputDir:'', localTempDir:'', agentApkPath:'', enterpriseSourcePath:'', ...(data.settings||{})};
    settingKeys.forEach(k => $(k).value = state.settings[k] || '');
    if (data.effectiveAgentApkPath) $('agentApkPath').placeholder = `企业默认：${data.effectiveAgentApkPath}`;
    if (data.enterprise?._enterpriseSource) $('enterpriseSourcePath').placeholder = `当前生效：${data.enterprise._enterpriseSource}`;
    status('设置已读取。修改后点击右上角“保存设置”。');
  }
  async function save(){
    settingKeys.forEach(k => state.settings[k] = $(k).value.trim());
  ...
  $('pickEnterpriseSourcePath').addEventListener('click', () => pick('enterpriseSourcePath', {mode:'file', title:'选择 enterprise-source.json', filter:'JSON 文件 (*.json)|*.json|所有文件 (*.*)|*.*'}));
